...
const apostar = (monto) => {
  ...
  // Verificar si el jugador tiene suficientes fichas para apostar
  if (jugador[fichas[monto]] > 0) {
    jugador[fichas[monto]] -= 1;             // Restamos una ficha del tipo seleccionado
    jugador[`apuestaFichas${monto}`] += 1;


    // Dibujamos la ficha apostada en la mesa
    dibujarFicha(divFichasApostadas, monto);

    // Actualizamos el monedero del jugador
